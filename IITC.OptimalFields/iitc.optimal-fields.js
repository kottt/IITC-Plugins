...
// PLUGIN START ///////////////////////////////////////////////////////
		...
		window.plugin.OptimalFields.checkPortals = function(polylineCoords, p1, p2, color1, color2, debug) {
			...
			if (!isCorrectDirection(a, b, toLatLng(__portalsCache[portalsInArea[0]]._latlng))) {
				a = toLatLng(p2), b = toLatLng(p1);
			}

			var ab = __lib.computeHeading(a, b);
			var ba = __lib.computeHeading(b, a);

			function getAngleInfo(id) {
				var portal = __portalsCache[id];
				var latlng = toLatLng(portal._latlng);

				return {
					angle: getPointAngle(latlng, this.point, this.edge),
					latlng: portal._latlng,
					id: id
				};
			}
			var aAngles = portalsInArea.map(getAngleInfo.bind({ point: a, edge: ab }));
			var bAngles = portalsInArea.map(getAngleInfo.bind({ point: b, edge: ba }));


			function comapreAngles(x, y) { return x.angle - y.angle; }
			aAngles.sort(comapreAngles);
			bAngles.sort(comapreAngles);


			function getId(item) { return item.id; }
			var arrA = aAngles.map(getId);
			var arrB = bAngles.map(getId);
